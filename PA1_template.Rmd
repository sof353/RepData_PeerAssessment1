...
```{r most active interval, results='hide'}
mostActive <- format(meanStepsByInt$time[which.max(meanStepsByInt$mean)], format='%H:%M %p') 
```

Most active time of the day is at `r mostActive`
...
```{r Mean Steps Per Day with all NAs substitutes for interval means}
...
The affect on the daily mean number of steps taken when ingnoring NAs compared to substituting NAs with average interval values is `r dailyMeanDelta` steps.  (Note: A positive number indicates that the mean is higher in the dataset with substituted values in place of NAs.)  
...
```{r create factor variable called jour for assigning weekday and weekend type to date}
...
```{r create plot for activity as a function of time interval and weekday or weekend}
